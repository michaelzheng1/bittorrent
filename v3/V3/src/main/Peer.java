 ...
 */
	...
	boolean recieveHandle() {
		while(in_buf.size() > 3) {
			...
			if(id == 0) {
				...
				if(this.cur_piece.idx != piece_idx) {
					return true;
				}
	...
	void makeRequestBlockMsg() {
		...
		List<BlockInfo> blockInfoLs = cur_piece.getBlockInfoLs();
		for(BlockInfo info : blockInfoLs) {
			...
			if(Settings.LOGGING) System.out.println(this.log_head + "Piece_idx: " + cur_piece.idx + " Send block request:"  + info.toString());
		}
	}
